<template>
    <div style="position: relative">
        ...
        <div style="position: absolute; width: 100%; height: 100%">
            <gl-template v-for="pair in AllComponents" :key="pair[0]" :ref="GlcKeyPrefix + pair[0]">
...
type AnyItemConfigs = RowOrColumnItemConfig[] | StackItemConfig[] | ComponentItemConfig[];
...
const props = defineProps({
    ...
    MapComponents = new Map<ComponentContainer, { refId: number; glc: typeof GlTemplate }>(),
    ...
    router.afterEach((route: RouteLocation) => {
        ...
        if (url && url !== '/') {
            if (routeChildren[url]) {
                const cntnr = Array.from(MapComponents.entries()).find(
                    ([key, value]) => value.refId === routeChildren[route.fullPath]
                )?.[0];
...
onMounted(() => {
    ...
    if (props.router)
        GLayout.on('activeContentItemChanged', (ci: ComponentItem) => {
            let url =
                ci.componentType === 'route' ? (<{ url: string }>ci.container.state)?.url : '/';
            if (router.currentRoute.value.fullPath !== url) router.replace(url);
        });
